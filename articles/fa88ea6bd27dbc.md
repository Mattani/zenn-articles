...
| [Contargo/redmine_oidc](https://github.com/Contargo/redmine_oidc) | OSS(GPL v2) | 最終コミット: 2024-05-14 | Redmine 5は対応。Redmine 6の対応表記なし | 中（ドキュメントは多いがRedmine6に未対応の可能性） |
...
| [kontron/redmine_oauth](https://github.com/kontron/redmine_oauth) | OSS(GPL v3) | 最終コミット: 2026-03-06（v4.0.4 リリース） | v3.0.1でRedmine 6.1対応の記述あり | 中〜容易（活発でドキュメント充実） |
...
あとは、プラグインを導入するにあたって、活発に更新されていることも重要なため `enricohuang/redmine_oidc` または`kontron/redmine_oauth`がプラグインの有力候補となります。`Contargo/redmine_oidc`は、記事執筆時点では Redmine 6 に対応していないようです。もし今後対応されたら、同様に試してみる候補となり得ます。
