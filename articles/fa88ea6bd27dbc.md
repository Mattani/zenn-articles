...
- **OmniAuth Strategy**: Ruby の `OmniAuth` フレームワークで使われる「strategy」（プロバイダごとの認証プラグイン）。OmniAuth 自体は OAuth2/OIDC の抽象を提供するが、OAuth2 に認証仕様が含まれないため、各 Strategy 側で「ユーザー識別情報の取得」「UserInfo API の差異吸収」「署名検証の欠如への対処」「Discovery/JWKS 未対応の補完」などを個別実装する必要があり、プロバイダごとに挙動が異なって複雑になりやすい。
