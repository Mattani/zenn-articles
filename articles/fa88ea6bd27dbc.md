...
以下、現時点でみつかった主なOAuth2/OIDCプラグインについて比較してみました。
...
上記で示したプラグインについて、機能の観点でも比較してみました。

| プラグイン | 認証方式 | Discovery / JWKS | PKCE | ユーザ同期 / 自動作成 |
| --- | --- | --- | --- | --- |
| [planio-gmbh/omniauth-redmine-oauth2](https://github.com/planio-gmbh/omniauth-redmine-oauth2) | OAuth2 (OmniAuth strategy) | ー | ー | 手動（自動作成の記載なし） |
| [Contargo/redmine_oidc](https://github.com/Contargo/redmine_oidc) →[jirutka/redmine_oidc](https://github.com/jirutka/redmine_oidc) | OIDC | 対応 | 未対応 | 自動登録・メール一致のオプションあり（マッピング機能あり） |
| [enricohuang/redmine_oidc](https://github.com/enricohuang/redmine_oidc) | OIDC | 対応 | 未対応 | 自動登録・自己リンク・メール一致など豊富なユーザ管理機能あり |
| [kontron/redmine_oauth](https://github.com/kontron/redmine_oauth) | OAuth2 / OIDC 両対応 | 対応(OIDCのみ) | 対応済み（3.0.2（2025-01-09）以降） | Redmine の Self-registration に依存する挙動（設定次第で登録/リンク） |

### 今回調査しなかったプラグイン

以下のプラグインも見つけましたが、更新が10年以上前。複数のフォークがあり更新されているものもありましたが詳細調査は割愛しました。
このプラグインのフォークやその他のプラグインで、同様に比較したほうがよいものがあればコメントおねがいいたします。

* [suer/redmine_oauth_provider](https://github.com/suer/redmine_oauth_provider)  
* [twinslash/redmine_omniauth_google](https://github.com/twinslash/redmine_omniauth_google)  
...
* OAuth2プラグイン: 本来は「認可フレームワーク」であり、認証（SSO）用途にそのまま流用すると、挙動やセキュリティ設計のために工夫や追加実装が必要になる。OAuth2 には認証仕様が含まれないため、ユーザー識別情報の取得方法やセキュリティ要件がプロバイダごとに異なり、実装が複雑になりやすい。その結果、運用面でも影響が出る可能性を否定できない。
* OIDCプラグイン: OAuth2 を認証用途に拡張した仕様で、Discovery/JWKS 等によりプロバイダ連携がシンプルに行えるため、短時間で試せる。（実装・運用ともに比較的容易と考えられる）

プラグインを導入するにあたっては、活発に更新されていることも重要なため `enricohuang/redmine_oidc` または`jirutka/redmine_oidc`がまず試してみるプラグインの有力候補となります。Redmine のようなサーバサイド Web アプリでは PKCE は必須ではなく、PKCE 非対応が即リスクになるわけではない。ただし、PKCE は OAuth2 の最新のセキュリティベストプラクティスであり、対応しているプラグインの方が将来性と安全性の面で望ましいため、今後の対応が期待されます。

`kontron/redmine_oauth`は、機能としては現時点では一番充実しているように見えます。PKCEや複数プロバイダ対応しているものも現時点ではこれだけです。機能面を優先するならこちらになりますが、ただ、OAuth2/OIDC両対応で実装がかなり複雑になっているとおもわれ、それが原因で導入や運用への影響があるかもしれません。

## 気づき・今後について

本記事執筆のうえで、私自身、SAML、OAuth2、OIDCや各プラグインの実装方式について調査してかなり理解が進みました。
歴史的な技術の移行をふまえて、あらためて現時点でSSOを実現するにはOIDCプラグインを選ぶのがよいということになります。
ただ、OIDCプラグインといっても複数の実装がでています。本記事はあくまで机上での調査によるものですので、実際にやってみたわけではありません。
今後、実際にやってみたらまたご紹介しようと思います。
...
* **OmniAuth Strategy**: Ruby の OmniAuth フレームワークで使われる「strategy」（プロバイダごとの認証プラグイン）。OmniAuth は認証フローの共通インターフェース（エントリポイントやコールバック処理の枠組み）を提供するが、OAuth2 には認証仕様が含まれないため、各 Strategy 側で「ユーザー識別情報の取得」「UserInfo API の差異吸収」「署名検証の欠如への対処」「Discovery/JWKS 未対応の補完」などを個別に実装する必要があり、プロバイダごとに挙動が異なって複雑になりやすい。
* **OIDC (OpenID Connect)**: OAuth2 の上に乗る「認証のための標準仕様」。Authorization Code Flow に加えて、Discovery（設定自動取得）や ID Token（JWT 署名付きのユーザー情報）など、認証に必要な要素がプロトコルとして定義されているため、プロバイダ間の差異が少なく、実装が安定しやすい。
このため、OmniAuth のように Strategy 側で個別実装を大量に書く必要がなく、近年の SSO では最も採用されやすい方式。
